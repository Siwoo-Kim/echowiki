...
public class UnitTestScopeElement {

    @Test
    public void unitTestParsedString() {
        ScopeExpression scopeExpression;
        //Element element = scopeExpression.evaluate();

    }
    public static final String ENCODED_TEXT =
        "=== {@doc} ===\n" +
        "{@echo1}: 정식 문법이 아니며 지원 중단 가능성이 있는 비권장 문법입니다.\n" +
        "\n" +
        "{@echo2}\n" +
        "이 문법은 글자의 배경에 {+:주석}을 넣는 기능입니다. (텍스트 뿐만 아니라 테이블의 셀 배경으로 적용 또한 가능함) <헥스 코드 1, 2> 자리에 자신이 넣고 싶은 여섯 자리의 [[헥스 코드]]들을 찾아서 입력해 주세요.\n" +
        "\n" +
        "그러데이션의 여백을 조절하고 싶다면 위 문법에 margin: (세로 여백 조절 숫자)px (가로 여백 조절 숫자)px; 을 추가로 입력하여 조절해 주세요.\n" +
        "\n" +
